...
- The pricing duration slider indexes the ordered `PLAN_CODES` catalog directly: one, two, and three days; one week; then one through twelve months. Keep all durations inside that single discrete slider instead of restoring separate short-plan buttons.
- Duration scale labels are plain text aligned to their slider positions; do not render day, week, or month labels as separate pill controls. The pricing contact panel intentionally offers email without a WhatsApp action.
- Platform support and legal contact are email-only through `support@pokarov.co.il`; do not publish a personal operator phone or restore WhatsApp links. Public business profiles may still show the business phone entered by each owner, while legacy WhatsApp columns and analytics values remain only for historical compatibility.
- Mobile navbar layout keeps only the linked logo and rotating audience text at the RTL start, without a visible `פה קרוב` wordmark, and a compact action group at the opposite edge; account details live in the navigation sheet below 1440px, and favorites render only when a working `onFavoritesOpen` handler is supplied. Reduced-motion mode must swap complete audience phrases without typing or a cursor instead of freezing on the first phrase.
- Mobile navbar search, favorites, and menu controls must remain at least 44×44 px at the 320, 390, and 430 px regression widths.
...
- New purchases use `listing_1d`, `listing_2d`, `listing_3d`, `listing_7d`, and `listing_1m` through `listing_12m`; exact day plans use immutable `plan_days`, month plans use immutable `duration_months`, and every new listing settlement snapshots the expiry before/after state for newest-first refunds.
- Server plan reads accept database pricing only when the complete immutable catalog and whole-shekel ₪20–₪250 ladder validate together; otherwise `resolvePlanCatalog` falls back to the application catalog as one unit so partially applied migrations cannot produce mixed checkout prices.
- Public and admin pricing pages are force-dynamic so a successful admin catalog update is visible without a redeploy; do not turn those pages back into build-time snapshots.
...
- Negative HYP callback writes must remain scoped to `status = 'pending'` so a delayed decline or failed verification cannot overwrite a concurrently succeeded or refunded settlement.
- Settlement uses HYP's provider-carried `Order`/`uniqueid` as the canonical attempt identifier and rejects any mismatch with local return-URL aliases. Checkout must charge the trigger-snapshotted `payment_attempts.amount_agorot` only after it exactly matches the catalog price shown to the buyer.
...
- Signed-photo failures must resolve to an empty URL and the branded `SafeBusinessImage` fallback; never restore stored public URLs, expired signed URLs, or arbitrary external image fallbacks.
...
- Policy-only ownership/content helpers live in the non-exposed `private` schema. `get_my_businesses` is the intentional authenticated public RPC; public review reads exclude `user_id`, owner payment reads exclude raw provider/auth fields, and stored callback diagnostics are allowlisted with card data reduced to the last four digits.
...
- Keep the Playwright global teardown's Supabase helper import behind `RUN_DESTRUCTIVE=1`; public/auth-only suites must not load destructive credentials or cleanup code.
...
- The `first-20-3m` launch campaign reserves one of 20 places atomically when an authenticated owner creates a self-service draft through 2026-12-31. The business stays private until manager approval; that approval starts three calendar months, sets the listing active, and must never spend review time from the free period.
- Public campaign availability comes only from the read-only aggregate `promotion_campaigns` row, but the public API and UI expose only the fixed 20-business cap and boolean eligibility—never live claimed or remaining counts. Per-business reservation and activation fields are server-managed and reach owners only through `get_my_businesses`; never add them to public business column grants.
- The empty-platform example must render through the shared `BusinessCard`, stay explicitly labelled as a demo, remain client-only with inert actions, and disappear as soon as any real public business exists. Filter/search zero-results must never substitute the demo.
...
- Production has `20260716071745_add_two_three_day_listing_plans.sql` applied; preserve that exact version so local migration history remains aligned before pushing the bounded price ladder.
- Production has `20260716091547_enforce_duration_price_ladder.sql` applied; preserve that exact version so the catalog, checkout trigger, and service-role-only admin pricing RPC stay aligned with migration history.
- Production has `20260716091648_move_policy_helpers_private.sql` applied; preserve that exact version so public review/payment/analytics/coupon grants and the private policy-helper schema are not replayed.
- The coordinated rollout completed in the order pricing migration `20260716091547`, compatible app deployment, then privacy migration `20260716091648`. Do not roll the app back across that privacy cutover without a compensating database migration.
- Supabase leaked-password protection is disabled, admin access is still a shared signed-secret session without individual MFA, and rate limits are per-instance memory rather than durable production controls.
...
- Duration prices form a whole-shekel, strictly increasing ₪20–₪250 ladder and admin edits are committed atomically through a service-role-only invoker-rights RPC.
