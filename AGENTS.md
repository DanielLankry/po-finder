...
- The pricing duration slider indexes the ordered `PLAN_CODES` catalog directly: one day, two days, three days, one week, then one through twelve months. Keep all durations inside that single discrete slider instead of restoring separate short-plan buttons.
...
- Production has `20260716071745_add_two_three_day_listing_plans.sql` applied; preserve that version so future pushes retain the two-day and three-day catalog rows and ordering.
