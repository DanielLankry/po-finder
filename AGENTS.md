...
- Public campaign availability comes only from the read-only aggregate `promotion_campaigns` row. Per-business reservation and activation fields are server-managed and reach owners only through `get_my_businesses`; never add them to public business column grants.
- The empty-platform example card must stay explicitly labelled as a demo, contain no invented rating or review, and disappear as soon as any real public business exists. Filter/search zero-results must never substitute the demo.
...
## Known Issues
- Production must receive `20260811085142_first_twenty_business_promotion.sql` before the campaign-enabled app is promoted; the API intentionally degrades to no offer when the aggregate table is absent.
