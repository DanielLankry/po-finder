...
- Never run against the production Supabase project ref `ymqlqdhelsocibhnanjy` or any `pokarov.co.il` base URL.
...
| `paid-active-owner` | `business_owner` | `seedPaidActiveBusiness` | Prove an approved, active, future-expiry listing appears on dashboard and public surfaces. | Bypass payment settlement for entitlement arithmetic checks. |
...
| `expired-paid-owner` | `business_owner` | `expireBusinessListing` after a real grant | Prove expired rows stay visible to the owner for billing continuity but disappear from public APIs, details, sitemap, and owner-authenticated public requests. | Let owner extend `expires_at` or change `is_active`. |
...
| Scenario | Setup | Assertion | Existing coverage |
| --- | --- | --- | --- |
| Unpaid owner creates first draft | Authenticated owner inserts inactive unverified business. | Insert succeeds; row has `is_verified = false`, `is_active = false`, and `expires_at = null`. | `unpaid-business-owner.spec.ts` |
| Unpaid owner cannot create unlimited drafts | Same owner attempts second insert. | Insert fails through RLS/trigger guard. | `unpaid-business-owner.spec.ts` |
| Free draft is private | Request public list and detail for draft. | `/api/businesses` omits row; `/businesses/:id` returns `404`. | `unpaid-business-owner.spec.ts` |
| Approved paid grant activates listing | Pending business is admin-approved, then settled with `listing_6m`. | Payment is `succeeded`; business is verified, active, and future-expiring. | `paid-listing-lifecycle.spec.ts` |
| Paid listing is public | Query anonymous API and detail page. | Public list includes row; detail page returns success. | `paid-listing-lifecycle.spec.ts`, `paid-business-owner.spec.ts` |
| Expired paid listing is private publicly | Force `expires_at` into the past. | Public API, detail page, sitemap, and signed-in owner public API all omit the row. | `paid-listing-lifecycle.spec.ts` |
| Expired owner keeps continuity reads | Read through owner client and billing page after expiry. | Owner can read business/payment data and dashboard billing state. | `paid-listing-lifecycle.spec.ts` |
| Owner cannot tamper lifecycle columns | Owner updates `expires_at` or `is_active`. | Updates fail and return no changed rows. | `paid-listing-lifecycle.spec.ts` |
| Stale profile status does not grant listing | Admin sets `users.subscription_status = active`; owner inserts second business. | Insert fails because consumed listing payment is the entitlement, not profile status. | `paid-listing-lifecycle.spec.ts` |
| Renewal refund ordering | Apply `listing_6m`, then `listing_2m`; preflight refund older grant. | Older refund preflight fails until newest grant is refunded. | `pricing-duration-products.spec.ts` |
...
```bash
RUN_DESTRUCTIVE=1 npx playwright test tests/destructive/pricing-duration-products.spec.ts --project=chromium-desktop
RUN_DESTRUCTIVE=1 npx playwright test tests/destructive/paid-listing-lifecycle.spec.ts --project=chromium-desktop
RUN_DESTRUCTIVE=1 npx playwright test tests/destructive/unpaid-business-owner.spec.ts --project=chromium-desktop
```

## Sentinel Review Checklist

- The matrix keeps production-blocking safeguards in `tests/utils/supabase-admin.ts`.
