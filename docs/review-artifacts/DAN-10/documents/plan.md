...
This plan supersedes the operational recommendations in `ADS-STRATEGY.md`,
`BUDGET-PLAN.md`, `CAMPAIGN-ARCHITECTURE.md`, `SIMPLE-ADS-PLAN.md`, and
`TRACKING-SETUP.md`. Those documents remain useful as creative source material,
but their subscription, price, domain, tracking-readiness, and campaign-structure
assumptions are stale. Po Finder now sells one-time listing durations in ILS,
uses `pokarov.co.il`, and already has consent-aware browser events.
...
### 3.1 Read-only G0 account inventory

Daniel confirmed on 2026-08-03 that Meta access is available. That attestation removes access availability as a planning blocker, but it is not evidence that the correct assets, permissions, or controls are configured and is not launch authority.

Before G1 or any campaign creation, publication, billing change, or spend, Pulse must inspect and record the following from an authenticated Meta session without changing state:

| Control | Required evidence | Pass condition |
|---|---|---|
| Business Portfolio | Exact name, ID, owning legal/business entity, status | Po Finder-owned portfolio is selected and active |
| Ad account | Exact name, ID, owner, status | Correct Po Finder account is active with no unexplained restriction |
| Facebook Page | Exact name, ID, owner, portfolio assignment | Correct Page is owned by or fully assigned to the portfolio |
| Instagram presence | Exact handle/ID, professional-account status, owner, Page linkage | Correct Po Finder identity is controlled by the portfolio and linked to the Page |
| Pixel/dataset | Exact name/ID, owner, ad-account connection, domain association, diagnostics | Production Pixel belongs to Po Finder and is connected only to approved assets |
| Domain | `pokarov.co.il` verification status and owner | Verified under the correct portfolio and associated to the approved data source |
| Role scope | Pulse/user/partner roles across portfolio, ad account, Page, Instagram, and dataset | Campaign-management/read access only; no finance, user-admin, or billing-change authority |
| Two-factor authentication | Portfolio requirement and enforcement/status for relevant users | 2FA is required and no relevant user is non-compliant |
| Currency/timezone | Ad-account currency and timezone | ILS and Asia/Jerusalem; mismatch is a launch stop because these settings can be difficult or impossible to change safely |
| Billing visibility | Whether billing state and payment-method presence are visible; no sensitive details copied | Budget owner can verify readiness; Pulse cannot add or alter billing |
| Spending controls | Current account limit, remaining amount, campaign limits/rules, and authority to remove/raise them | 300 ILS test-window account cap or approved equivalent can be set by the budget owner; Pulse cannot raise/remove it |

The canonical result belongs in the issue document `g0-verification`. Every row must be marked `pass`, `fail`, or `not verified`, with timestamped evidence and the actor who observed it. Screenshots/exports must redact personal identifiers and payment details. G0 passes only when every required control passes and the production measurement/consent checks in Section 12 also pass.

Access availability, plan approval, a configured payment method, or successful login alone never counts as G0 approval.
