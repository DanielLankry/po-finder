...
- [x] Unify today's hours, publication progress, profile completion and mobile discovery controls — 2026-09-09
  - Files modified: `app/api/account/progress/route.ts`, dashboard layout/home/profile/photos/schedule/events/billing pages, `app/MapPage.tsx`, `app/globals.css`, dashboard owner workspace/today/lifecycle components, dashboard sidebar, email verification panel, business card/list/popup, `lib/owner-progress.ts`, `lib/db/owned-businesses.ts`, `lib/hooks/useOwnerFormAnchor.ts`, `lib/utils/schedule.ts`, `tests/owner-progress.test.mjs`, `docs/quality/owner-clarity/`, `TASKS.md`, `AGENTS.md`.
  - Summary: Added an authenticated owner progress read model, five publication steps, six linked profile checks, explicit responsibility and next action, daily/weekly/overnight availability, DST-safe calendar dates, selected-business continuity, and a reserved mobile map/list control row. Promotion copy distinguishes free approval-based activation from optional paid extensions; no approval SLA is invented.
  - Validation: 166 native tests after integrating the latest email-notification changes, lint, TypeScript and production build; local browser checks for owner states and responsive discovery at 320–1440px. Authenticated database column grants were inspected read-only. No production test user, business, payment or promotion reservation was created. Evidence and verification limits: `docs/quality/owner-clarity/README.md`.

### Security and billing notifications
- [x] Enable branded password/email security notices and durable payment, renewal and refund messages — 2026-09-09
  - Files modified: `lib/email-templates.ts`, `lib/payment-email-template.ts`, `lib/payment-email-outbox.ts`, `app/api/payments/return/route.ts`, `app/api/admin/payments/[id]/refund/route.ts`, `app/api/admin/payment-emails/route.ts`, `app/api/cron/payment-emails/route.ts`, `app/admin/payment-emails/page.tsx`, `app/admin/layout.tsx`, `app/admin/businesses/page.tsx`, `scripts/build-security-email-templates.mjs`, `scripts/payment-email-catalog.mjs`, the three security templates under `supabase/templates/`, `supabase/migrations/20260909122231_payment_email_outbox.sql`, five `tests/payment-email-*.test.mjs`, `tests/admin/payment-emails.spec.ts`, `tests/utils/source-loader.mjs`, `package.json`, `package-lock.json`, `vercel.json`, `docs/payment-email-rollout.md`, `docs/audits/2026-09-09/payment-email-admin-mobile.png`, `AGENTS.md`, `TASKS.md`.
  - Summary: Security notices are enabled; billing events persist immutable messages with deduplication, bounded retries and a private admin delivery queue. Nine hosted Resend templates are published.
  - Validation: 24 focused Node/PostgreSQL tests, 13 related regressions, 12 admin desktop/mobile scenarios, lint and build. Four actual security emails and three labelled billing examples were delivered; no production card was charged.
